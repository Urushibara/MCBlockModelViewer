...
export class BlockMeshGroup extends THREE.Group {
    _modelLoader;
    _textureLoader; // MCTextureLoader のインスタンス
     ...
     */
    constructor({ modelLoader, textureLoader }) {
        super();
     ...
     * @param {Array<object>} modelDefs - 各モデル定義 { model: string, x?: number, y?: number, uvlock?: boolean }
     * @returns {Promise<void>}
     */
    async prepare(modelDefs) {
        ...
        console.log("[BlockMeshGroup] All required models and textures prepared.");
    }
     ...
     * @returns {Promise<void>}
     */
    async _prepareModelAndTextures(modelRef) {
        ...
        try {
            ...
            if (!modelData) {
                throw new Error(`Model data not found or failed to load for ${modelRef}`);
            }
            ...
            if (modelData.textures) {
                ...
                for (const textureName in modelData.textures) {
                    ...
                    textureLoadPromises.push(this._textureLoader.loadTexture(textureRef));
                }
            ...
            if (modelData.elements) {
                ...
                if (modelData.textures) {
                    for (const textureName in modelData.textures) {
                        ...
                        texturesForMCElementMesh[`#${textureName}`] = await this._textureLoader.loadTexture(textureRefPath);
     ...
     * @private
     * @param {object} modelDef - モデル定義 (主に位置と回転情報の取得用)
     */
    _addFallbackCube(modelDef) {
    ...
    dispose() {
        ...
        // MCTextureLoader のキャッシュをクリアする場合は、MCTextureLoader のインスタンスから明示的に呼び出す。
        // 例: this._textureLoader.clearCache();
